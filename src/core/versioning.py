...
def save_section_version(filename: str, section_id: str, user: str, content: str):
    ...
    version_filename = f'versions/{filename}/{filename}.section_{section_id}.{timestamp}.{user}.md'
    ...
    os.makedirs(f'versions/{filename}', exist_ok=True)
...
def get_all_versions(filename: str):
    ...
    filename = os.path.basename(filename)
    pattern = f'versions/{filename}/{filename}.section_*.*.md'
...
def rollback_section(filename: str, section_id: str, timestamp: str, user: str):
    ...
    version_filename = f'versions/{os.path.basename(filename)}/{os.path.basename(filename)}.section_{section_id}.{timestamp}.{user}.md'
